...
class CategoriesController(Controller):   
    ...
    @route(f"{CHATBOT_OPTIONS_URL}/report/name/<name>", type='http', auth='none', methods=['GET'], csrf=False, cors="*")
    def geocode_get_address(self, name):
        report = request.env['city.report'].sudo().search_read([('name', '=', name)], ["name", "category_id", "subcategory_id"], limit=1)
        if report:
            response_body = {
                "result": report[0],
